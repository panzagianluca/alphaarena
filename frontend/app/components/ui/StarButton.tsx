...
export function StarButton({
  ...
  return (
    <button
      ...
      ref={pathRef}
      className={cn(
        "relative z-[3] overflow-hidden h-12 px-7 py-2.5 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 group/star-button cursor-pointer",
        className,
      )}
      {...props}
    >
      ...
      />
      <div
        className="absolute inset-0 border-[#c9a84c]/30 z-[4] overflow-hidden rounded-[inherit]"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        <StarBackground color={backgroundColor} />
      </div>
      <span className="z-10 relative text-[#c9a84c] text-[14px] font-medium">
        {children}
      </span>
    </button>
  );
